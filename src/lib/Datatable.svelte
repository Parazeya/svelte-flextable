<script lang="ts">
	import { type DataHandler, Search, RowsPerPage, RowCount, Pagination, ExportButtons } from "$lib/core";
	...
	export let xlsx = false;
	export let csv = false;
	export let json = false;

	export let exportFileName = "Datatable";
	export let exportHeaders = null;
...
<section bind:clientWidth>
	<header class:container={xlsx || csv || json} style="padding-top: 16px">
		<div><ExportButtons {handler} {xlsx} {csv} {json} name={exportFileName} headers={exportHeaders} /></div>
	</header>
