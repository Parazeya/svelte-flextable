...
<div in:fade>
	...
	<ul>
		...
		<li class:active={current === "serverSide"} on:click={() => (current = "serverSide")}>Server-side</li>
	...
	<section class="thin-scrollbar" class:code={current === "code" || current === "data"} class:table={current === "result"}>
		...
		{:else if current === "serverSide"}
			<CopyButton code={serverSide} />
			<div class="code thin-scrollbar">
				<pre class="language-javascript">
{@html Prism.highlight(serverSide, Prism.languages.javascript)}
			</pre>
			</div>
		{:else}
